...
main :: proc() {
  ...
  // And I'm too lazy to handle cross-platform CTRL+C signal
  ...
  for !rl.WindowShouldClose() {
    ...
    case .RECEIVE:
      ...
      // We don't need to destroy the packet we created
      // because ENet takes its ownership and uses reference counting to destroy it
      enet.host_broadcast(server, 0, packet)
        
    case .DISCONNECT:
      full_message := fmt.tprintf(
        "USER %s disconnected",
        common.format_enet_address(event.peer.address),
      )
      // enet.packet_create() will memcpy the data we provided. (unless .NO_ALLOCATE flag provided)
