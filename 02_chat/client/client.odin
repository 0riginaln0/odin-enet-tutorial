...
main :: proc() {
  ...
  for !rl.WindowShouldClose() && !quit {
    ...
    if rl.IsMouseButtonPressed(.LEFT) {
      ...
      if rl.CheckCollisionPointRec(mouse_pos, send_button_rect) {
        ...
        if len(nickname) == 0 {
          ...
          // The ENet library will destroy the packet automatically on its own via reference counting.
        }
      }
    }
    ...
    rl.DrawText(nickname_cstr, i32(nickname_rect.x + 10), i32(nickname_rect.y + 10), 20, rl.WHITE)
    rl.EndScissorMode()
      ...
      for i := len(messages) - 1; i >= start_index; i -= 1 {
        ...
        if len(msg) > 0 {
          ...
          if text_y >= chat_history_rect.y {
            ...
            rl.DrawText(msg_cstr, i32(chat_history_rect.x + 10), i32(text_y), 18, rl.WHITE)
          }
        }
      }
    } else {
